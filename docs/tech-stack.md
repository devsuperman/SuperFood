...
| ADR-13 | API docs via the built-in `Microsoft.AspNetCore.OpenApi` document generator + Scalar (`Scalar.AspNetCore`) UI at `/scalar/v1`, Development-only | Swashbuckle/Swagger UI; NSwag; no interactive docs | `AddOpenApi()`/`MapOpenApi()` ship in the framework and already produce the spec Blazor tooling reads — no third-party generator to keep current with each .NET bump. Scalar renders that spec, bundles its own JS/CSS locally (no CDN, works offline), and is one line to map. Swashbuckle's ASP.NET Core 10 support has lagged. A `BearerSecuritySchemeTransformer` adds the JWT scheme so "Authorize" works against real endpoints; docs stay off in non-Development environments so the spec isn't exposed publicly. |
